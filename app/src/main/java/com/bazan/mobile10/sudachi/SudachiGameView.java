...
public class SudachiGameView extends SurfaceView implements SurfaceHolder.Callback, Runnable {
    //虫のでかさ
    private final float BUG_L_SCALE = 0.5F;
    //虫の最大出現数
    private int bugMaxCount = 50;
    //虫の発生スピード
    private int bugMakeSpeed = 1;
    //ゲーム時間
    private long gameTime = 10000;
    ...
    public SudachiGameView(Activity activity) {
        super(activity);
    ...
    private void drawTimeBar() {
        ...
        double ritu = jikan / (double) gameTime;
        if(ritu > 1) {
            ritu = 1;
        }
