...
public class BearGameView extends SurfaceView implements SurfaceHolder.Callback, Runnable {
    //虫のでかさ
    private final float BUG_L_SCALE = 0.5F;
    //虫の最大出現数
    private int bugMaxCount = 1;
    //虫の発生スピード
    private int bugMakeSpeed = 0;
    //ゲームHP
    private long hp = 10000;
    ...
    public BearGameView(Activity activity) {
        super(activity);
        ...
        bmp[BAR] = readBitmap(activity, "hpbar");
        holder = getHolder();
    ...
    private void drawHPBar() {
        double ritu = damage / (double) hp;
        if (ritu > 1) {
            ritu = 1;
        }
        //double barsize = ((barWidth) * ritu);
        //ここをコメントアウトするとバーを増加、コメントアウトで減少
        double barsize = ((barWidth - 50) * ritu);
        barsize = barWidth - barsize;
        if (startTime != 0) {
            g.drawBitmapF(bmp[BAR], 50, 50, (int) barsize, 100);
        }
    }
    ...
    private void hpRecov() {
        if(damage > 0){
            damage = damage - hpRecovery;
        }
